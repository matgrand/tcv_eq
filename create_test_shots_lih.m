...
OUT_DIR = 'test_shots_lih'; % more space available
...
if ~exist(OUT_DIR, 'dir')
  ...
  fprintf('Output directory already exists. Old files deleted: %s\n', OUT_DIR);
end
...
for i = 1:length(shots)
  ...
  try
    ...
    % print final sizes
    fprintf('\tFinal sizes -> Fx:%s, Iy:%s, Br:%s, Bz:%s, \n\tBm:%s, Ff:%s, Ft:%s, Ia:%s, Ip:%s, Iu:%s, rBt:%s, rq:%s, zq:%s\n', ...
